...
class BiCNNLSTMTranstion(nn.Module):
    def __init__(self, vocabularySize, embedDimension, minNgrams, maxNgrams, charInputDim, charOutDim, hiddenDim, tagSize, init_embedding, language_weight):
...
# pack the correct previous target variables so that the loss function can be appropriately calculated
        ...
        if languageId == 0:
            ...
            return self.nll_loss(prob_output, t_out.data).sum() / count * self.language_weight, predIndex.data
...
class BiCNNLSTMTranstionOutput(nn.Module):
    def __init__(self, vocabularySize, embedDimension, minNgrams, maxNgrams, charInputDim, charOutDim, hiddenDim, tagSize, init_embedding, language_weight, tagSizeAux):
        super(BiCNNLSTMTranstionOutput,self).__init__()
...
# forward pass to get the output from Bi-LSTM layer
        ...
        loss, predIndex = self._decode(seq_bilstm_out_order, target, batchSize,  maxLength, mask, use_gpu, languageId)

        return loss, predIndex

    def _decode(self, seq_output, correctTarget, batchSize, maxLength, mask, use_gpu, languageId):
        if languageId == 0:
            predictionList = torch.LongTensor(batchSize, maxLength).fill_(0)
            ...
            for time_step in range(maxLength):
                init_predictions = torch.FloatTensor(batchSize, self.tagSizeAux).fill_(0.0)
                ...
                wordPrePrediction = self.outputLayerAux(inputToOutLayerWord)
                wordPrediction = self.logsoftmaxAux(wordPrePrediction)
                # batch * tagSize
                ...
                # need to calculate loss only for the non-padded words
                newWordScores = torch.FloatTensor(count, self.tagSizeAux).fill_(0.0)
                newTarget = torch.LongTensor(count).fill_(0)
                k = 0
