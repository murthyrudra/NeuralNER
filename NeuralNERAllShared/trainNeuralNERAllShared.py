...
def main():
	...
	trainCorpusAux, trainLabelsRawAux, maxTrainLengthAux = readCoNLL(train_path_aux, charVocabulary, targetVocabulary, args.ner_tag_field_l2, embedding_vocab)
	...
	network = BiCNNLSTMTranstion(inputVocabulary.__len__(), embeddingDimension, min_filter_width, max_filter_width, charVocabulary.__len__(), num_filters, hidden_size, targetVocabulary.__len__() , word_embedding, args.beta)
	...
	for epoch in range(1, num_epochs + 1):
		...
		with tqdm(total= ( len(trainCorpusRawSorted) + len(trainCorpusRawSortedAux) ) ) as pbar:
			...
			for l1,l2 in zip_longest(l1_indices, l2_indices):
				...
				if l2 is not None:
					x_input, batch_length, current_batch_size, current_max_sequence_length, y_output, mask, y_prev = constructBatch([trainCorpusRawSortedAux[l2]], [trainLabelsRawSortedAux[l2]], inputVocabulary, targetVocabulary, charVocabulary, max_filter_width, args.use_gpu)
