...
# similarly construvt the output target labels, everySentence in every batch one-by-one
    ...
    batch_target_prev = torch.FloatTensor(len(batchSentences), max_sequence_length, len(tagDict)).fill_(0)
    mask = torch.FloatTensor(len(batchSentences), max_sequence_length).fill_(0)
    index = 0

    for i in range(len(batchSentences)):
        for j in range(len(batchSentences[i])):
            batch_target[i][j] = tagDict[batchLabels[i][j]]

            if j != 0:
                batch_target_prev[i][j][tagDict[batchLabels[i][j-1]]] = 1.0
            mask[i][j] = 1.0
            index = index + 1
