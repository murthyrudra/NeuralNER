...
class DeepLayer(nn.Module):
	...
	def forward(self, x_in):
		...
		output = self.nonLinear2(self.linear2(self.nonLinear1(self.linear1(x_in))))
		return output

class BiCNNLSTM(nn.Module):
	def __init__(self, vocabularySize, embedDimension, minNgrams, maxNgrams, charInputDim, charOutDim, hiddenDim, tagSize, init_embedding ):
	...
	def loss(self, x, length_of_sequence, batchSize, maxLength, target, mask):
...
class BiCNNLSTMTranstion(nn.Module):
	def __init__(self, vocabularySize, embedDimension, minNgrams, maxNgrams, charInputDim, charOutDim, hiddenDim, tagSize, init_embedding, freeze ):
		...
		self.embedLayer = nn.Embedding(self.vocabularySize, self.embedDimension, padding_idx=0)
		self.embedLayer.weight = Parameter(torch.Tensor(init_embedding))
		self.embedLayer.require_grad = True
	...
	def loss(self, x, length_of_sequence, batchSize, maxLength, target, mask, y_prev, use_gpu):
...
# forward pass to get the output from Bi-LSTM layer
	...
	def forward(self, x, length_of_sequence, batchSize, maxLength, target, mask, y_prev, use_gpu):
...
# forward pass to get the output from Bi-LSTM layer
		...
		loss, predIndex, probs = self._decode(seq_bilstm_out_order, target, batchSize,  maxLength, mask, use_gpu)

		return loss, predIndex, probs
	...
	def _decode(self, seq_output, correctTarget, batchSize, maxLength, mask, use_gpu):
		...
		predictionProbs = torch.FloatTensor(batchSize, maxLength, self.tagSize).fill_(0.0)
		...
		for time_step in range(maxLength):
			...
			wordPredictionP = self.softmax(wordPrePrediction)
			# batch * tagSize
			...
			for j in range(batchSize):
				...
				for k in range(self.tagSize):
					predictionProbs[j][time_step][k] = wordPredictionP[j][k].item()

		return loss, predictionList, predictionProbs

	def forwardOnline(self, x, length_of_sequence, batchSize, maxLength, mask, use_gpu):
# batchSize is always 1, it's an hack but still we'll go with it
		embedOut = self.embedLayer(x[0])
...
# forward pass to get the output from Bi-LSTM layer
		...
		predIndex = self._decodeOnline(seq_bilstm_out_order, batchSize,  maxLength, mask, use_gpu)

		return predIndex

	def _decodeOnline(self, seq_output, batchSize, maxLength, mask, use_gpu):
		predictionList = torch.LongTensor(batchSize, maxLength).fill_(0)
	...
	def forwardOnly(self, embedPre,  x, length_of_sequence, batchSize, maxLength, languageId, use_gpu):
# batchSize is always 1, it's an hack but still we'll go with it
		charOut = []
# extract the sub-word features and the input is batchSize, sequenceLength, (numberOfCharacters * characterFeatures)
		for i,l in enumerate(self.charLayers):
...
# concatenate word representation and subword features
		# finalWordOut = torch.cat( [finalCharOut, embedOut, langOutput], 2)
		finalWordOut = torch.cat( [finalCharOut, embedPre], 2)
...
# forward pass to get the output from Bi-LSTM layer
		...
		count = 0
		for i in range(batchSize):
			for j in range(length_of_sequence[i].data.item()):
				count = count + 1

		predIndex = self._decodeOnly(seq_bilstm_out_order, batchSize, maxLength, use_gpu)

		return predIndex, embedPre.squeeze(), finalCharOut.squeeze()

	def _decodeOnly(self, seq_output, batchSize, maxLength, use_gpu):
		predictionList = torch.LongTensor(batchSize, maxLength).fill_(0)
		...
		for time_step in range(maxLength):
			...
			for j in range(batchSize):
				predictionList[j][time_step] = index[j].data.item()

		return predictionList

	def getWordEmbeddings(self, x, use_gpu):
# batchSize is always 1, it's an hack but still we'll go with it
		embedOut = self.embedLayer(x)

		return embedOut

	def getCharEmbeddings(self, x, use_gpu):
# batchSize is always 1, it's an hack but still we'll go with it
		charOut = []
# extract the sub-word features and the input is batchSize, sequenceLength, (numberOfCharacters * characterFeatures)
		for i,l in enumerate(self.charLayers):
		...
		finalCharOut = finalCharOut.squeeze()
		return finalCharOut
